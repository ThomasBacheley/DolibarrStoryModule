...
/**
 * \file    story/admin/setup.php
 * \ingroup story
 * \brief   Story setup page.
 */
...
require_once '../lib/story.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "story@story"));
...
$arrayofparameters = array(
	'STORY_MYPARAM1'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
	'STORY_MYPARAM2'=>array('type'=>'textarea','enabled'=>1),
	//'STORY_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
	//'STORY_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
	//'STORY_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
	//'STORY_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'STORY_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'STORY_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/story/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'STORY_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'STORY_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = storyAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "story@story");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("StorySetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'STORY_'.strtoupper($myTmpObjectKey).'_ADDON';
