...
/**
 * 	\defgroup   story     Module Story
 *  \brief      Story module descriptor.
 *
 *  \file       htdocs/story/core/modules/modStory.class.php
 *  \ingroup    story
 *  \brief      Description and activation file for module Story
 */
include_once DOL_DOCUMENT_ROOT.'/core/modules/DolibarrModules.class.php';

/**
 *  Description and activation class for module Story
 */
class modStory extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleStoryName' not found (Story is name of module).
		...
		// Key used in llx_const table to save module status enabled/disabled (where STORY is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array(
                // '/story/css/story.css.php',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				//   '/story/js/story.js.php',
			),
		...
		// Example: this->dirs = array("/story/temp","/story/subdir");
		$this->dirs = array("/story/temp");

		// Config pages. Put here list of php page, stored into story/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@story");
		...
		$this->langfiles = array("story@story");
		...
		//$this->automatic_activation = array('FR'=>'StoryWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(1 => array('STORY_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
		//                             2 => array('STORY_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
		// );
		$this->const = array();
		...
		if (!isset($conf->story) || !isset($conf->story->enabled)) {
			$conf->story = new stdClass();
			$conf->story->enabled = 0;
		}

		// Array to add new pages in new tabs
		$this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@story:$user->rights->story->read:/story/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		// $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@story:$user->rights->othermodule->read:/story/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		...
		$this->dictionaries=array(
			...
			'tabcond'=>array($conf->story->enabled, $conf->story->enabled, $conf->story->enabled)
		);
		*/

		// Boxes/Widgets
		// Add here list of php file(s) stored in story/core/boxes that contains a class to show a widget.
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'storywidget1.php@story',
			//      'note' => 'Widget provided by Story',
		...
		$this->cronjobs = array(
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => '$conf->story->enabled',
			//      'priority' => 50,
			//  ),
		);
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->story->enabled', 'priority'=>50),
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->story->enabled', 'priority'=>50)
		// );
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/story/sql/');
		if ($result < 0) {
		...
		//$result1=$extrafields->addExtraField('story_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'story@story', '$conf->story->enabled');
		//$result2=$extrafields->addExtraField('story_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'story@story', '$conf->story->enabled');
		//$result3=$extrafields->addExtraField('story_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'story@story', '$conf->story->enabled');
		//$result4=$extrafields->addExtraField('story_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'story@story', '$conf->story->enabled');
		//$result5=$extrafields->addExtraField('story_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'story@story', '$conf->story->enabled');
